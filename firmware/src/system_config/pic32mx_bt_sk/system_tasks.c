...
static void _Recognition_Tasks (void);
// *****************************************************************************
// *****************************************************************************
...
void SYS_Tasks ( void )
{
    ...
    xTaskCreate((TaskFunction_t) _Recognition_Tasks,
                "Recognition Tasks",
                configMINIMAL_STACK_SIZE*10, NULL, 1, NULL);
...
//    while( (xAccelQueue = xQueueCreate(1, sizeof(ACCEL_XYZf)) ) == 0);
    while( (xAccelRawQueue = xQueueCreate(100, sizeof(ACCEL_XYZ_RAW)) ) == 0);
...
static void _Recognition_Tasks (void)
{
    while( (xAccelZMQueue = xQueueCreate(FRAME_SIZE, sizeof(ACCEL_ZM)) ) == 0);
    while( (xActiveMotionIntervalQueue = xQueueCreate(FRAME_SIZE*50, sizeof(ACCEL_ZM)) ) == 0);
