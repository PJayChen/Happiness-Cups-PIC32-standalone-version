...
{
	int nr_class = model->nr_class;
	double _dec_values[nr_class*(nr_class-1)/2];
    double *dec_values;
	if(model->param.svm_type == ONE_CLASS ||
	   model->param.svm_type == EPSILON_SVR ||
	   model->param.svm_type == NU_SVR)
        dec_values = _dec_values;
...
void preprocessing() {
    ...
    ACCEL_ZM acc_zm;
    while (uxQueueSpacesAvailable != 0) {
        if (xQueueReceive( xAccelRawQueue, &( acc_xyz_raw ), NULL ) != pdTRUE) {
            return;
        } else {
            acc_zm.acc_z = acc_xyz_raw.acc_z;
            acc_zm.acc_magnitude = (int) (sqrt(powi(acc_xyz_raw.acc_x, 2) + powi(acc_xyz_raw.acc_y, 2) + powi(acc_xyz_raw.acc_z, 2)) - GRAVITY_ACC);
            xQueueSend(xAccelZMQueue, &acc_zm, NULL);
        }   
    }
}


int getFramePower(const ACCEL_ZM* const frame_acc) {
    int power = 0;
    int i = 0;
    for (;i<FRAME_SIZE;i++) {
        power += (int) powi(frame_acc->acc_magnitude, 2);
    }
...
int getFrame(ACCEL_ZM *frame_acc) {
    if (uxQueueSpacesAvailable(xAccelZMQueue) != 0) return 0;
    // Collected # samples achieve FRAME_SIZE
    // Framing the samples, 64 samples per frame, 50% overlap
    int i;
    for (i=0;i<FRAME_SIZE/2;i++) {
        xQueueReceive( xAccelZMQueue, &( frame_acc[i] ), NULL );
    }
    for (;i<FRAME_SIZE;i++) {
        xQueuePeek( xAccelZMQueue, &( frame_acc[i] ), NULL );
    }
    return 1;
}

void storeAcvieMotionInterval(const ACCEL_ZM* const frame_acc) {
    int i = 0;
    for (; i<FRAME_SIZE;i++)
        xQueueSend(xActiveMotionIntervalQueue, &frame_acc[i], NULL);
}

// converts feature vector into libsvm tf format, namely, svm_node.
void featureExtractoin(struct svm_node *featureVector) {
    int nr_acc;
    int i=0;
    
    double meanZ = 0, meanM = 0;
    double varZ = 0, varM = 0;
    double uT_z = 0, uT_m = 0;
    double meanZ_frame = 0, lastMeanZ_frame = 0;
    double meanM_frame = 0, lastMeanM_frame = 0;
    int count = 0;
    
    nr_acc = FRAME_SIZE*50 - uxQueueSpacesAvailable(xActiveMotionIntervalQueue);
    ACCEL_ZM acc_zm[nr_acc];
    
    for (;i<nr_acc;i++) {
        xQueueReceive( xActiveMotionIntervalQueue, &( acc_zm[i] ), NULL );       
    }
    
    for (i=0;i<nr_acc;i++) {
        meanZ += acc_zm[i].acc_z;
        meanM += acc_zm[i].acc_magnitude;
    }    
    meanZ /= nr_acc;
    meanM /= nr_acc;
    
    for (i=0;i<nr_acc;i++) {
        varZ += (meanZ - acc_zm[i].acc_z)*(meanZ - acc_zm[i].acc_z);
        varM += (meanM - acc_zm[i].acc_magnitude)*(meanM - acc_zm[i].acc_magnitude);
        if (count == FRAME_SIZE - 1) {
            meanZ_frame /= FRAME_SIZE;
            meanM_frame /= FRAME_SIZE;
            
            uT_z += abs(meanZ_frame - lastMeanZ_frame);
            uT_m += abs(meanM_frame - lastMeanM_frame);
            ...
            count++;
            meanZ_frame += abs(acc_zm[i].acc_z);
            meanM_frame += abs(acc_zm[i].acc_magnitude);
        }
    }
    varZ /= nr_acc;
    varM /= nr_acc;
    ...
    featureVector[0].index = 1;
    featureVector[0].value = meanZ;
    featureVector[1].index = 2;
    featureVector[1].value = meanM;
    featureVector[2].index = 3;
    featureVector[2].value = varM;
    featureVector[3].index = 4;
    featureVector[3].value = varZ;
    featureVector[4].index = 5;
    featureVector[4].value = uT_m;
    featureVector[5].index = 6;
    featureVector[5].value = uT_z;
    featureVector[6].index = -1;
    featureVector[6].value = 0;
    
    
}

int identifyMotion(void) {
	struct svm_node featureVector[7];
    int result;
    
    featureExtractoin(featureVector);
    
    scale(featureVector);
    
    result = svm_predict(&svmModel, featureVector);
    
    return result;
} 


__attribute__((always_inline)) void Recognition_Tasks ( void )
{
    static int state = DETECT_MOTION;
    static ACCEL_ZM frame_acc[FRAME_SIZE];
    int identify_motion;
    
    // Fetch acceleration raw data, and calculate magnitude
    preprocessing();
    
    switch(state) {
        case FRAMING:
            if (getFrame(frame_acc) == 1)          
                state = DETECT_MOTION;
            else 
                state = FRAMING;
        ...
        case DETECT_MOTION:          
            ...
            if (getFramePower(frame_acc) > POWER_THRESHOLD) {
                storeAcvieMotionInterval(frame_acc);
        ...
        case FIND_END_FRAME:
            ...
            if (getFrame(frame_acc) != 1) break;
             // Determine whether the end-point(frame) of motion or
             // not
            if (getFramePower(frame_acc) > POWER_THRESHOLD/2) {
                storeAcvieMotionInterval(frame_acc);
                state = FIND_END_FRAME;
            } else {
                // the end-point(frame) of motion is found.
                state = IDENTIFY_MOTION;
                // remove remained 50% frame in the queue.
                int i;
                for (i=0;i<FRAME_SIZE/2;i++) {
                    xQueueReceive( xAccelZMQueue, &( frame_acc[i] ), NULL );
                }
            ...
            break;
        case IDENTIFY_MOTION:
            identify_motion = identifyMotion();
            //send recognition result to LED control task
            xQueueOverwrite(xDataReceivedFromBTQueue, &identify_motion);
            vTaskDelay(50 / portTICK_PERIOD_MS);
            state = FRAMING;
            break;
        default:state = FRAMING;
    }  
}
